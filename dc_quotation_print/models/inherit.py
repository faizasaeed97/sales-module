...
class DCQuotationSaleOrderInherit(models.Model):
            ...
            for line in work_line:
                if line.zones:
                    if last_zone == -1 or last_zone != line.zones.id:
                        ...
                        self._cr.execute("select sum(total_cost) as total_cst from scope_work_line where saleorder="+ str(self.id) +" and zones="+ str(line.zones.id) +";")
    ...
    def getNoZoneTotal(self):
        work_line = self.env['scope.work.line'].search([('zones', '=', False), ('saleorder', '=', self.id)])
        noZoneTotal = 0
        for line in work_line:
            noZoneTotal = noZoneTotal + line.total_cost
        return noZoneTotal
...
of any invoice, sales order, acknowledgement, purchase order or confirmation issued by Client.</p>"""
    ...
    if_cont_amount = fields.Boolean("Show Contigecy Amount ?", default=True)
    if_supervision_required = fields.Boolean("Show Supervision Required ?", default=True)
